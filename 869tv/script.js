...
let username = ""; let isHost = false; let currentHost = "Rovan"; let replyTo = null; let isSyncing = false; let syncTimeout = null;

function emitSync(type) { if (!isHost || isSyncing) return; isSyncing = true;
...
const near = Math.abs(video.currentTime - data.time) < 0.3; if (!near) video.currentTime = data.time;

if (data.type === "play" && video.paused) { video.play().catch(() => {}); } else if (data.type === "pause" && !video.paused) { video.pause(); } });

video.addEventListener("play", () => { if (!isSyncing && isHost && !video.paused) emitSync("play"); });

video.addEventListener("pause", () => { if (!isSyncing && isHost && video.paused) emitSync("pause"); });

video.addEventListener("seeked", () => { if (!isSyncing && isHost) emitSync("seek"); });
